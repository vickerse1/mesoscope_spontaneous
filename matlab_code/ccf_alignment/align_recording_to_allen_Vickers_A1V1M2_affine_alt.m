function tform = align_recording_to_allen_Vickers_A1V1M2_affine_alt(image,alignareas,affine)
...
subplot(221);
imCCF=imagesc(dorsalMaps.dorsalMapScaled);
imCCF.AlphaData=0.9;
axis equal off;
hold on;
...
saveas(gcf,strcat(outFile,'_A1V1M2_CCF_affine_alt.png'))

imCCF=imread(strcat(outFile,'_A1V1M2_CCF_affine_alt.png'));
invtform=invert(tform);
imCCFinv=imwarp(imCCF,invtform);
figure;imshow(imCCFinv)
saveas(gcf,strcat(outFile,'_A1V1M2_CCF_affine_alt_inverse.png'))

save(strcat(outFile,'_A1V1M2_affine_alt'))
end